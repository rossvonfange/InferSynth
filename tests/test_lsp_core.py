...
def test_completion_items_contain_keyword_and_param():
    ...
    # opamp-gain-noninverting, opamp-gain-inverting, and (batch 5)
    # instrumentation-amp-3opamp all declare a "gain" param;
    # completion_items dedups by (label, is_param), first entry wins in
    # sorted cell-key order -> instrumentation-amp-3opamp ("ins" < "opa").
    gain_item = next(i for i in items if i.label == "gain" and i.is_param)
    assert gain_item.detail == "core/instrumentation-amp-3opamp@0.1.0"
    assert "2.0..1000.0" in gain_item.documentation
